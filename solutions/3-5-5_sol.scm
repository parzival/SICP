  ...
  (let ((prg (make-pseudo-random-generator))
    ...
    (define (rand request)
      ...
      (define (reset new-val)
        (set-random-seed new-val prg)
          new-val
          ; 'reset
        )

      (cond
        ((eq? request 'generate)  (generate))
        ((and (list? request) (eq? (car request) 'reset)) (reset (cadr request)))
        ((and (pair? request) (eq? (car request) 'reset)) (reset (cdr request)))
        (else
         (error "Unknown argument for rand :" request)
         )
        )
      )
    
    
    (stream-map rand request-stream)
    ;(stream-filter number? (stream-map rand request-stream))
...
(define rg-stream (rand-gen rng-test-requests))
...
(define rs-congen-1 (rand-gen constant-gen))
(define rs-congen-2 (rand-gen constant-gen))
...
(define rs-congen-s-1 (rand-gen (con-gen-with-seed 7)) )
(define rs-congen-s-2 (rand-gen (con-gen-with-seed 7)) )
...
(define rand-out-stream (rand-gen (con-gen-with-seed 50000)))
...
(define nr-stream (rand-gen rng-requests))
(define nr2-stream (rand-gen rng-requests-2))
...
; This will scale a number in [0,1] to the specified range
(define (random-in-range low high rng-stream)
   (let ((range (- high low)))
    (+ low (* range (stream-car rng-stream)))
    )
  )
...
(define (estimate-integral-stream P x1 x2 y1 y2)
    (define (experiment-stream rand-stream)
     ...
     (let ((total-area (* (- x2 x1) (- y2 y1)))
       ...
       (scale-stream (monte-carlo (experiment-stream (rand-gen constant-gen)) 0 0) total-area)
    )
  )
