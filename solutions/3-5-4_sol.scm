...
(define test-2nd-order (solve-2nd 4 -4 1 2 timestep))  ; y'' - 4y' + 4y = 0 -> y = (e^(2t))
...
(define (gen-solve-2nd f y0 d-y0 dt)
  (define y (integral (delay dy) y0 dt))
  (define dy (integral (delay ddy) d-y0 dt))
...
(define test-generic-2nd-order (gen-solve-2nd lin2nd 1 2 timestep))
...
(define osc1 (gen-solve-2nd (make-damped-duffing 0.06) 1 initial-vel timestep))
(define osc2 (gen-solve-2nd (make-damped-duffing 0.20) 1 initial-vel timestep))
...
(define test-circuit (RLC 1.0 1.0 0.2 0.1))
...
(define test-circuit-ud  (RLC 1.0 0.5 0.2 0.1))  ; underdamped

(define test-circuit-cd (RLC 2.0 0.5 0.5 0.1)) ; critically damped

; overdamped
(define test-circuit-expl (RLC 10.0 0.2 1.0 0.1))  ; what should happen vs what does 
(define test-circuit-fix (RLC 10.0 0.2 1.0 0.01)) ; (how to fix it)
