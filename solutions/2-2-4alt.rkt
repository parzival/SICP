...
; Turn painter as defined in exercise into one that works with loaded file
...
(print-vect (sub-vect v2 v1)) ; (-2.0, -1.5)  (subtraction should yield vectors in opposite directions)
(newline)
(print-vect (scale-vect  2.0 v2)) ; (-4, 7)
(newline)
(print-vect (scale-vect -0.5 v1)) ; (0, -2.5)  (note FP error for 0.0 * -0.5)
(newline)
...
; (Segments-painter defined as part of included library)
...
(paint (below sg-wave-painter cross-painter))
(displayln "diamond beside diamond below wave")
(paint (below (beside diamond-painter diamond-painter) sg-wave-painter))
