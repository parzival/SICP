...
(define (averager a b c)
  (let ((sumab (make-connector))
        (numvals (make-connector))
        )
    (constant 2.0 numvals)
    (multiplier c numvals sumab)
    (adder a b sumab)
    )
  )
...
; This only works one-way; the square cannot be set independently
    ...
    (test-group
     ...
     (test-group
      ...
      (test-group-with-cleanup
       "b sets a"
       (begin
         (set-value! bs 25 'test)
         (test-assert (has-value? bs))
         (test-assert (has-value? as))
         (test-connector-= bs 25)
         )
       (clear-all)
       )
      (test-group-with-cleanup
       "Forgetting b clears a when set"
       ; relies on previous test working properly
       (begin
         (set-value! bs 49 'test)
         (test-assert (has-value? as))
         (forget-value! bs 'test)
         (test-assert (not (has-value? as)))
         )
       (clear-all)
         )
      (test-group-with-cleanup
       ...
       (begin
         (set-value! as 7 'test)
         (test-assert (has-value? bs))
         (forget-value! as 'test)
         (test-assert (not (has-value? bs)))
         )
       (clear-all)
         )
      )
     (test-group
      ...
      (test-group-with-cleanup
       ...
       (begin
         (set-value! bs 2 'test)
         (test-connector-= as 1.414213)
         )
       (clear-all)
       )
      (test-group-with-cleanup
       "Zero-values computed correctly (a)"
       (begin
         (set-value! as 0 'test)
         (test-connector-= bs 0)
         )
      ...
      (test-group-with-cleanup
       "Zero-values computed correctly (b)"
       (begin
         (set-value! bs 0 'test)
         (test-connector-= as 0)
         )
